...
import { Target, AlertTriangle, TrendingUp, Plus } from 'lucide-react';
import {
...
export default function BudgetsView({ initialBudgets, currentExpenses, categories }: any) {
    ...
    return (
        <div className="min-h-screen bg-background pb-40">
            {/* Header */}
            <div className="glass-nav sticky top-0 z-20 px-6 py-4 flex items-center justify-between border-b">
            ...
            <div className="container mx-auto px-4 pt-8 max-w-5xl space-y-8">
                ...
                <div className="bg-black text-white rounded-[32px] p-8 relative overflow-hidden shadow-2xl animate-fade-in">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="h-3 bg-neutral-800 rounded-full overflow-hidden">
                            <div
                                className={`h-full transition-all duration-1000 ease-out ${globalProgress > 100 ? 'bg-accent-500' : globalProgress > 80 ? 'bg-amber-400' : 'bg-secondary-500'}`}
                ...
                {budgetData.filter((b: any) => b.status === 'exceeded').map((b: any) => (
                    <div key={b.id} className="bg-accent-500/10 border border-accent-500/20 rounded-[24px] p-5 flex items-start gap-4 animate-shake">
                        <div className="p-2 bg-accent-500/15 rounded-xl text-accent-600 dark:text-accent-400">
                            <AlertTriangle className="w-6 h-6" />
                        </div>
                        <div>
                            <h3 className="font-bold text-accent-700 dark:text-accent-400">¡Presupuesto Excedido!</h3>
                            <p className="text-sm text-accent-600/80 dark:text-accent-400/80 mt-1">
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-center px-2">
                        <h3 className="text-sm font-black text-muted-foreground uppercase tracking-widest">Estado por Categoría</h3>
                    </div>

                    {budgetData.length === 0 ? (
                        <div className="bg-card rounded-[32px] p-10 text-center border border-dashed border-border">
                            <Target className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
                            <p className="text-muted-foreground font-bold mb-4">No tienes presupuestos definidos</p>
                            <button
                                onClick={handleNew}
                                className="bg-primary text-primary-foreground px-6 py-3 rounded-2xl font-bold text-sm"
                            >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {budgetData.map((b: any, index: number) => (
                                <div
                                    ...
                                    className="bg-card p-5 rounded-3xl border border-border shadow-sm relative overflow-hidden group hover:border-primary/30 hover:shadow-medium cursor-pointer transition-all active:scale-[0.98]"
                                ...
                                >
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-center gap-3 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <h4 className="font-bold text-foreground truncate">{b.category.name}</h4>
                                        ...
                                        <div className={`shrink-0 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${b.status === 'exceeded' ? 'bg-accent-500/15 text-accent-600 dark:text-accent-400' :
                                                b.status === 'warning' ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400' :
                                    ...
                                    <div className="h-2.5 bg-muted rounded-full overflow-hidden">
                                        <div
                                            className={`h-full rounded-full transition-all duration-1000 ${b.status === 'exceeded' ? 'bg-accent-500' :
                ...
                {/* TRENDS CHART */}
                <div className="bg-card rounded-[32px] p-6 border border-border shadow-sm mt-8">
                    <div className="flex items-center gap-2 mb-6">
                        <TrendingUp className="w-5 h-5 text-primary" />
                        <h3 className="font-bold text-foreground">Análisis: Presupuesto vs Real</h3>
                    </div>
