...
export default function BanksManager({ initialBanks, userId, onBanksUpdate }: Props) {
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      ...
      {/* Formulario de alta: Bottom Sheet (100% ancho en móvil, centrado en desktop) */}
      <Dialog open={showForm} onOpenChange={(open) => { if (!open) resetForm(); }}>
        <DialogContent className="w-full sm:max-w-md p-0 gap-0">
          <DialogHeader className="px-5 py-4 border-b border-border">
            <DialogTitle>Añadir Banco</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleAddBank} className="p-5 space-y-5">
            <div>
            ...
            {error && (
              <div className="flex items-center gap-2 p-4 bg-accent-500/10 rounded-xl border border-accent-500/20">
                <AlertCircle className="w-5 h-5 text-accent-600 dark:text-accent-400 shrink-0" />
                <p className="text-sm text-accent-700 dark:text-accent-400 font-medium">{error}</p>
              </div>
            )}

            <div className="flex gap-3 pb-[max(0.25rem,env(safe-area-inset-bottom))]">
              <button
                type="button"
                onClick={resetForm}
      ...
      {/* Edición: mismo patrón Bottom Sheet */}
      <Dialog open={!!editingBankId} onOpenChange={(open) => { if (!open) setEditingBankId(null); }}>
        <DialogContent className="w-full sm:max-w-md p-0 gap-0">
          <DialogHeader className="px-5 py-4 border-b border-border">
          ...
          {editingBank && (
            <div className="p-5 space-y-5">
              <input
                ...
                className="w-full px-4 py-2.5 border border-border rounded-xl bg-muted/60 text-sm font-medium text-foreground focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/10"
                autoFocus
              />

              <div>
                <div className="h-12 rounded-xl transition-all mb-3" style={{ backgroundColor: editBankColor }} />
              ...
              <select
                ...
                className="w-full px-3 py-2.5 border border-border rounded-lg text-sm font-medium bg-muted/60 text-foreground focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/10"
              >
                <option value="">Sin logo</option>
                {Object.entries(BANK_LOGOS).map(([name, url]) => (
                  <option key={name} value={url}>{name}</option>
              ...
              <div className="flex gap-2 pb-[max(0.25rem,env(safe-area-inset-bottom))]">
                <button
                  type="button"
                  onClick={() => handleUpdateBank(editingBank.id)}
                  disabled={loading}
                  className="flex-1 px-4 py-2.5 bg-secondary-500 text-white rounded-lg text-sm font-semibold hover:bg-secondary-600 transition-colors disabled:opacity-50"
                >
            ...
            {banks.map((bank) => (
              ...
              >
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  {/*
                   * El logo de la mayoría de bancos ES un logotipo con el nombre incluido
                   * (wordmark): repetir el nombre al lado es una etiqueta duplicada. Solo se
                   * muestra el nombre en texto cuando no hay logo (fallback con icono genérico).
                   */}
                  {!bank.logo_url && (
                    <span className="font-semibold text-foreground truncate">{bank.name}</span>
                  )}
                </div>
                ...
                <div className="flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity shrink-0">
                  <button
                    ...
                    title={bank.logo_url ? bank.name : undefined}
                  >
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => handleDeleteBank(bank.id, bank.name)}
                    className="p-2 hover:bg-accent-500/10 rounded-lg text-accent-600 dark:text-accent-400 transition-colors"
                  >
